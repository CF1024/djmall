...
<script type="text/javascript">
    ...
    $.validator.setDefaults({
        submitHandler : function() {
            ...
            token_post(
                "<%=request.getContextPath()%>/user/address/update?TOKEN="+getToken(),
                $("#fm").serialize(),
                function(data) {
                    layer.close(index);
                    if(data.code !== 200){
                    ...
                    layer.msg(data.msg, {offset: '230px', icon: 6, time: 2000},
                        function(){
                            parent.window.location.href = "<%=request.getContextPath()%>/user/address/toShowAddress?TOKEN="+getToken();
                        }
                    );
                }
            );
        ...
        <form id="fm">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="receiverName" id="receiverName" value="${address.receiverName}" placeholder="请输入收货人" class="layui-input">
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="phone" id="phone" value="${address.phone}" placeholder="请输入手机号" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
           ...
           <%-- <div class="layui-form-item">
                ...
                <select name="province" onclick="getCity(this.value)" id="province" class="form-control">
                    ...
                    <c:forEach items="${areaList}" var="area">
                        <option value="${area.id}" <c:if test="${address.province == area.id}">selected</c:if>>
                                ${area.areaName}
                        </option>
                    </c:forEach>
                </select>
                <select name="city" onclick="getCounty(this.value)" id="city" class="form-control">
                    <c:forEach items="${areaList}" var="area">
                        <option value="${area.id}" <c:if test="${address.city == area.id}">selected</c:if>>
                                ${area.areaName}
                        </option>
                    </c:forEach>
                </select>
                <select name="county"  id="county" class="form-control">
                    <c:forEach items="${areaList}" var="area">
                        <option value="${area.id}" <c:if test="${address.county == area.id}">selected</c:if>>
                                ${area.areaName}
                        </option>
                    </c:forEach>
                </select>
